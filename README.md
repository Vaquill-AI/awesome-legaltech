...
- [Caddi](https://www.trycaddi.com) - **[AI-Native]** Agentic automation for law firm back-office operations, specified by demonstration rather than configuration: a supervisor agent both discovers repetitive multi-tool processes across the firm and, from a screen-shared and narrated walkthrough, authors a hybrid agent (deterministic code, with AI reserved for judgment steps) that runs work such as matter intake, DMS filing, and pre-billing across 80+ integrations including iManage, NetDocuments, Clio, Aderant, and Salesforce.
