...
- [keihyo-cases](https://github.com/eoylab/keihyo-cases) - **[🇯🇵 Japan]** MCP server over Consumer Affairs Agency enforcement orders under the Act against Unjustifiable Premiums and Misleading Representations (景品表示法) — 措置命令 and 課徴金納付命令 — with the provisions cited in each order and a source URL on every record. Structured deterministically from the agency's own published pages; states explicitly that it contains no judgement of its own. Also JSON/JSONL/CSV. Data under the Public Data License 1.0, code MIT.
- [jftc-actions](https://github.com/eoylab/jftc-actions) - **[🇯🇵 Japan]** MCP server over Japan Fair Trade Commission cease-and-desist orders (Antimonopoly Act) and Subcontract Act (取適法, formerly 下請法) recommendations, with a source URL per action. Personal names of sole proprietors are removed from the structured fields; unparsed announcements are kept in a separate file rather than filled in by guesswork. Data under the Public Data License 1.0, code MIT.
...
- [SpecterAI](https://www.specterlaw.ai/) - **[AI-Native] [🇩🇪 Germany]** Source-backed legal research, document analysis, drafting, and case workflows built for German law and professional users.
...
- [MiCAR Whitepaper Linter](https://github.com/sebastianfoerste/micar-whitepaper-linter) - **[🇪🇺 EU]** Deterministic linter for crypto-asset white papers under MiCAR (EU) 2023/1114: 36 rules mapped to Annex I-III with pinpoint citations, a CI action, and a browser playground.
